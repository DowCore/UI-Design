<template>
<div class="vdr" :style="style" :class="`${(active || isActive) ? 'active' : 'inactive'} ${contentClass ? contentClass: ''}`" @mousedown="bodyDown($event)" @touchstart="bodyDown($event)" @touchend="up($event)">
    <slot></slot>
    <div v-for="stick in sticks" :key="stick" class="vdr-stick" :class="['vdr-stick-' + stick]" @mousedown.stop.prevent="stickDown(stick, $event)" @touchstart.stop.prevent="stickDown(stick, $event)" :style="vdrStick(stick)">
    </div>
</div>
</template>
...
export default {
    ...
    props: {
        ...
        parentLimitation: { //是否受父容器限制，true不能超过父容器
            type: Boolean,
            default: false
        },
...
.vdr.active:before {
    ...
    pointer-events: none; /* 伪元素不能点击，防止子元素的单击失效 */
}
